...
* **📁 `02_eda_estatistica_e_visualizacao.ipynb`**: Análise Exploratória de Dados (EDA). Uso de estatística descritiva para identificar distribuições assimétricas e tratamento matemático de outliers. Visualização de dados e teste de hipóteses usando Seaborn/Matplotlib. Preparação para o notebook 3 com Método IQR.
* **📁 `03_ml_kmeans_clustering.ipynb`**: Aplicação de Machine Learning. Padronização de features, validação matemática da quantidade de clusters e visualização de alta dimensionalidade. Método do Cotovelo.
...
1. Clone este repositório:
   ```bash
   git clone https://github.com/vini-gm/airbnb-analise-e-segmentacao.git
   ```
2. Instale as dependências listadas no requirements.txt:
   ```bash
   pip install -r requirements.txt
   ```
3. Execute os notebooks sequencialmente (01 -> 02 -> 03) para reproduzir o pipeline. Os dados originais encontram-se na pasta ``data/raw`` e os processados em ``data/processed``.
