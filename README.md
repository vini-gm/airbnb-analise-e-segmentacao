...
Este projeto constrói um **pipeline de análise de dados ponta a ponta** utilizando dados reais do Airbnb (Rio de Janeiro) para extrair insights estatísticos sobre precificação e aplicar algoritmos de Machine Learning Não-Supervisionado para segmentar o mercado imobiliário. 
Além disso, busca:
* Entender a composição dos imóveis;
* Analisar valores das diárias;
* Analisar valores dos imóveis;
* Identificar imóveis fora da curva;
...
* **📁 `01_coleta_e_manipulacao.ipynb`**: Foco em qualidade de dados. Extração, conversão de tipos (ex: limpeza de strings monetárias para `float`) e tratamento de valores nulos (NaN).
...
2. **Mapa de Percepção de Valor:** O cruzamento de dados geoespaciais com a métrica de avaliações revelou os bairros que entregam o melhor custo-benefício, mostrando que o maior valor percebido não está necessariamente nas áreas mais caras (Zona Sul tradicional).
...
*(Gráfico do Cotovelo)*
> `![Gráfico do Cotovelo](link_da_imagem_aqui.png)`

---

*(Mapa de Calor do Plotly)*
> `![Mapa de Calor](link_da_imagem_aqui.png)`
